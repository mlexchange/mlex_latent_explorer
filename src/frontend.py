...
def update_latent_vectors_and_clusters(submit_n_clicks, input_data, n_components):
    input_data = np.array(input_data)
    if (submit_n_clicks is None) or (input_data is None):
        raise PreventUpdate
    ## TODO: current only working for PCA
...
# )
# def update_umap_latent_vectors_and_clusters(submit_n_clicks, input_data, n_components, min_dist, n_neighbors):
#     input_data = np.array(input_data)
#     if (submit_n_clicks is None) or (input_data is None):
#         raise PreventUpdate
